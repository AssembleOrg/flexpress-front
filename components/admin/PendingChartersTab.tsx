...
function StandaloneVehicleCard({
  vehicle,
  onApprove,
  onReject,
  isPending,
}: StandaloneVehicleCardProps) {
          ...
          {docCount === 0 ? (
            ...
            <Box sx={{ display: "flex", gap: 1.5, flexWrap: "wrap", mb: 2 }}>
              {(vehicle.documents ?? []).map((doc) => (
                <Tooltip
                  key={doc.id}
                  title={DOC_TYPE_LABEL[doc.type] ?? doc.type}
                >
                  <Box sx={{ position: "relative" }}>
                    <PrivateDocThumb
                      value={doc.fileUrl}
          ...
          {(charter.userDocuments?.length ?? 0) === 0 ? (
            ...
            <Box sx={{ display: "flex", gap: 1.5, flexWrap: "wrap", mb: 2 }}>
              {(charter.userDocuments ?? []).map((doc) => (
                <Tooltip
                  key={doc.id}
                  title={`DNI ${DOC_TYPE_LABEL[doc.side ?? ""] ?? doc.side ?? ""}`}
                >
                  <Box sx={{ position: "relative" }}>
                    <PrivateDocThumb
                      value={doc.fileUrl}
...
export function PendingChartersTab() {
  ...
  const [selectedCharter, setSelectedCharter] =
    useState<PendingCharterReviewItem | null>(null);
  const [rejectDialogOpen, setRejectDialogOpen] = useState(false);
  const [rejectionReason, setRejectionReason] = useState("");

  // Charter approve confirmation dialog
  const [approveCharter, setApproveCharter] =
    useState<PendingCharterReviewItem | null>(null);
  ...
  const [approveVehicleDialogOpen, setApproveVehicleDialogOpen] =
    useState(false);
    ...
    <Box>
      ...
      {vehiclesFromVerifiedCharters.length > 0 && (
        <Box mt={pendingCharters.length > 0 ? 5 : 0}>
          <Divider sx={{ mb: 3 }} />
          <Typography variant="h6" fontWeight={700} mb={3}>
            Vehículos pendientes de conductores verificados (
            {vehiclesFromVerifiedCharters.length})
          </Typography>
          <Stack spacing={2}>
      ...
      <Dialog
        ...
        onClose={() => !isApprovingAll && setApproveDialogOpen(false)}
        maxWidth="sm"
        fullWidth
      >
        ...
        <DialogContent>
          <Typography variant="body2" mb={2}>
            ¿Aprobar al conductor <strong>{approveCharter?.name}</strong>? Podrá
            ...
            <Box>
              ...
              <Stack spacing={0.75} mb={1.5}>
                {(approveCharter?.vehicles ?? []).map((v) => {
                  ...
                  const isRejected =
                    v.verificationStatus === VerificationStatus.REJECTED;
                  return (
                    <Stack
                      key={v.id}
                      direction="row"
              ...
              {(() => {
                ...
                return pendingCount > 0 ? (
                  <Alert severity="warning" sx={{ py: 0.5 }}>
                    Se aprobará el conductor y {pendingCount} vehículo
      ...
      >
        ...
        <DialogContent>
          ...
          <TextField
            ...
            helperText={
              rejectionReason.trim() === "" ? "El motivo es obligatorio" : ""
            }
          />
        </DialogContent>
        <DialogActions>
      ...
      <Dialog
        open={approveVehicleDialogOpen}
        onClose={() => setApproveVehicleDialogOpen(false)}
        maxWidth="xs"
        fullWidth
      >
        ...
        <DialogActions>
          <Button onClick={() => setApproveVehicleDialogOpen(false)}>
            Cancelar
          </Button>
          <Button
      ...
      >
        ...
        <DialogActions>
          <Button onClick={() => setVehicleRejectDialogOpen(false)}>
            Cancelar
          </Button>
          <Button
            ...
            variant="contained"
            disabled={
              !vehicleRejectionReason.trim() || verifyVehicleMutation.isPending
            }
          >
